...
Videos: H.264 at CRF 18 (visually lossless), keep original resolution and
audio, add faststart. Replace only if smaller.
"""
...
def compress_video(path: Path) -> None:
    """Visually lossless re-encode: H.264 CRF 17, copy audio, faststart.
    Falls back to faststart-only (-c copy) if the re-encode is larger."""
    old = path.stat().st_size
...
def main() -> None:
    ...
    print("\n=== VIDEOS (-c copy, +faststart, lossless) ===")
    for p in sorted(ASSETS.rglob("*")):
